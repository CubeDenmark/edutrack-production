...
// Define the emit to update the activePage and content
...
const userType = user.user_type; // 'prof', 'student', or 'parent'
...
// User dropdown state and management
...
// Method to update active page without full navigation
const updateActivePage = (href: string) => {
  ...
  router.visit(href, {
    ...
    data: {
      itemId: currentItem.value // Pass the current item ID if one is selected
    }
  });
};
...
// Define section titles based on user type
const sectionTitles = {
  prof: 'MY CLASSES',
  student: 'MY CLASSES',
...
// Function to get the appropriate section title
const getSectionTitle = () => {
  return sectionTitles[userType] || 'MY ITEMS';
};
