...
const updateActivePage = (href: string) => {
  ...
  router.visit(href, {
    ...
    data: {
      ...
      childId: currentChild.value // Pass the current child ID if one is selected (for parent user type)
    }
  });
};
...
const currentChild = ref<string | number>('all'); // Add currentChild ref for parent user type

// Provide the current section ID and child ID to child components
provide('currentSectionId', currentSection);
provide('currentChildId', currentChild); // Provide currentChild to child components
...
// Enhanced selectChild function to match selectSection functionality
const selectChild = (childId) => {
  ...
  // Emit the child ID to the parent component
  emit('update:content', 'child', childId);

  // Get the URL for the selected child
  ...
  currentUrl.searchParams.set('childId', childId.toString());
...
onMounted(() => {
  ...
  const childId = urlParams.get('childId'); // Check for child ID in URL
  ...
  if (childId) {
    currentChild.value = childId === 'all' ? 'all' : parseInt(childId);
  }

  // Listen for child-changed events from other components
  window.addEventListener('child-changed', (event: CustomEvent) => {
    const { childId, source } = event.detail;

    // Only process if the event didn't originate from this component
    if (source !== 'sidebar') {
      currentChild.value = childId;
    }
  });

  // Listen for section-changed events from other components
  window.addEventListener('section-changed', (event: CustomEvent) => {
