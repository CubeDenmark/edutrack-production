...
// const userType = ref('prof'); // Change this dynamically based on actual user type
const userItems = ref([]);
...
function addClass() {
  router.post('/classes', newClass, {
    onSuccess: (response) => {
      // Assuming your backend returns the created class, add it to the UI
      userItems.value.push({ ...newClass });
...
//   selectItem(userItems.value[userItems.value.length - 1])
// }

// function addClass() {
//   router.post('/classes', newClass, {
//     onSuccess: () => {
//       showAddClassModal.value = false; // Close modal on success
//     },
//     onError: (errors) => {
//       console.error(errors); // Handle validation errors
//     }
    ...
    >
      ...
      <div class="flex flex-col h-full overflow-hidden">
        ...
        <div class="flex-1 overflow-y-auto scrollbar-thin">
          ...
          <div v-bind="$attrs" class="max-h-[calc(100vh-400px)] overflow-y-auto scrollbar-thin">
            <nav class="p-2">
              <ul class="space-y-1">
  ...
  <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg w-full max-w-md p-6">
    ...
    <form @submit.prevent="addClass" class="space-y-4">
      ...
      <div>
        ...
        <input type="text" v-model="newClass.term" placeholder="e.g., Fall Semester 2023" class="input-style" />
      </div>
      ...
      <div>
        ...
        <div class="grid grid-cols-5 gap-2">
          <button
            ...
            :class="['h-10 w-10 flex items-center justify-center text-lg rounded-md', newClass.icon === icon ? 'bg-primary-100 dark:bg-primary-900 border-2 border-primary-500' : 'bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600']"
          >
            {{ icon }}
          </button>
        </div>
      ...
      <div>
        ...
        <textarea v-model="newClass.description" placeholder="Brief description of the class" class="input-style"></textarea>
      </div>

      <div>
        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Schedule</label>
        <input type="text" v-model="newClass.schedule" placeholder='e.g., ["Monday 10AM-12PM", "Wednesday 2PM-4PM"]' class="input-style" />
      </div>
      ...
      <div>
        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Professor ID</label>
        <input type="text" v-model="newClass.professor_id" placeholder="Enter professor ID" class="input-style" />
      </div>

      <div class="flex justify-end space-x-2 pt-4">
        <button type="button" @click="showAddClassModal = false" class="px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-md text-sm">
          Cancel
        </button>
        <button type="submit" class="px-4 py-2 bg-primary-600 text-white rounded-md text-sm">
          Add Class
        </button>
      </div>
    </form>
...
/* Input styles */
.input-style {
  @apply w-full px-3 py-2 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-primary-600 focus:border-primary-600;
}
