...
// Class sections data for professors and students
...
provide('currentChildId', selectedChild);
...
// Select child without page navigation (for parent view)
const selectChild = (childId) => {
  // Prevent re-selecting the same child to avoid infinite loops
  if (selectedChild.value === childId) {
    return;
  }

  selectedChild.value = childId;

  // For parent view, we'll use the same mechanism as section selection
  // but with a different event name
  emit('update:content', 'child', childId);

  // Update the URL with the child ID without navigating
  const url = new URL(window.location.href);
  if (childId === 'all') {
    url.searchParams.delete('childId');
    url.searchParams.delete('sectionId');
  } else {
    url.searchParams.set('childId', childId.toString());
    // Also set sectionId for compatibility with the unified approach
    url.searchParams.set('sectionId', childId.toString());
  }
  window.history.replaceState({}, '', url.toString());

  // Dispatch custom events that components can listen for
  ...
  // Also dispatch section-changed event for compatibility
  window.dispatchEvent(new CustomEvent('section-changed', {
    detail: {
      sectionId: childId,
      source: 'sidebar'
...
onMounted(() => {
  ...
  // Check if there's a section ID or child ID in the URL
  const urlParams = new URLSearchParams(window.location.search);
  const sectionId = urlParams.get('sectionId');
  const childId = urlParams.get('childId');

  if (userType === 'parent') {
    // For parent view, handle child ID
    if (childId) {
      selectedChild.value = childId;
    } else if (sectionId) {
      // For compatibility, also check sectionId
      selectedChild.value = sectionId;
    }
  } else {
    // For professor and student views, handle section ID
    if (sectionId) {
...
<div class="flex items-center justify-between p-4 border-b dark:border-gray-700">
  <div class="flex items-center space-x-2">
    <div class="bg-primary-600 p-1.5 rounded">
      <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 14l6.16-3.422a12.083 12.083 0 01.665 6.479A11.952 11.952 0 0012 20.055a11.952 11.952 0 00-6.824-2.998a12.078 12.078 0 01.665-6.479L12 14zm-4 6v-7.5l4-2.222" />
      </svg>
    </div>
  ...
  <button @click="toggleSidebar" class="md:hidden">
    ...
    <!-- Student-specific: Class Sections -->
        ...
        <div class="relative">
          ...
          >
            ...
            <div class="p-1">
              ...
              >
              <div>
    <svg v-if="darkMode" xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
