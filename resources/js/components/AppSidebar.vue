...
const closeDropdownOnClickOutsideSidebar = (event) => {
    ...
    const isOutsideDropdown = !event.target.closest('.dropdown-trigger') && !event.target.closest('.dropdown-menu');
...
const deleteClass = async (id) => {
    try {
        ...
        await router.delete(`/classes/${id}`, {
            preserveScroll: true, // Prevent routing to a blank page
            onSuccess: () => {
...
const editClass = (classItem) => {
    ...
    // Parse and assign the schedule properly
    try {
        if (Array.isArray(classItem.schedule)) {
            // Schedule is already an array
            scheduleEntries.value = classItem.schedule;
        } else if (typeof classItem.schedule === 'string') {
            // Schedule is a JSON string
            const scheduleData = JSON.parse(classItem.schedule);
            if (Array.isArray(scheduleData)) {
                // Ensure only valid entries with both day and time
                scheduleEntries.value = scheduleData.filter((entry) => entry.day && entry.time);
            } else {
...
function hasDuplicateSchedules(entries) {
    const seen = new Set();
...
function addClass() {
    ...
    if (hasDuplicateSchedules(scheduleEntries.value)) {
        formErrors.value.schedule = ['Duplicate schedule entries are not allowed (same day and time).'];
    ...
    if (isEditing.value) {
        ...
        router.post('/classes', newClass, {
            onSuccess: () => {
                fetchClasses().then(() => {
                    showAddClassModal.value = false;
                    displaySuccessMessage('Class added successfully');
                });
            },
            onError: (errors) => {
                console.error('Error adding class:', errors);
                formErrors.value = errors;
            },
        });
    }
    ...
    try {
        ...
        if (userType === 'parent' && data.children) {
            userItems.value = data.children.map((child) => ({
                ...
                avatar: child.avatar, // Default icon if not set in the database
            }));
            ...
            userItems.value = data.classes.map((cls) => ({
                ...
                schedule: cls.schedule, // Store the raw schedule data
                days: cls.days || [], // Store the days array if available
            ...
            // Dispatch an event to notify other components about the updated data
            window.dispatchEvent(
                new CustomEvent('classes-updated', {
                    detail: {
                        classes: userItems.value,
                    },
                }),
            );
        }
...
const parseTimeRange = (timeRange) => {
    if (!timeRange || typeof timeRange !== 'string') {
        return { startTime: '8:00 AM', endTime: '10:00 AM' };
    }
...
const proceedWithDeleteClass = async () => {
    if (classToDelete.value) {
        try {
            await deleteClass(classToDelete.value);
...
<template>
                    ...
                    <div class="scrollbar-thin max-h-[250px] space-y-2 overflow-y-auto pr-1">
                        <div v-for="item in userItems" :key="item.id" class="relative">
                            ...
                            >
                                ...
                                <div class="flex flex-grow cursor-pointer items-center" @click="selectItem(item)">
                            ...
                            <div
                                ...
                                class="dropdown-menu absolute right-0 z-10 mt-1 w-32 rounded-md border border-gray-200 bg-white shadow-md dark:border-gray-600 dark:bg-gray-700"
                            >
                                <button
                                ...
                                <button
                                    ...
                                    @click="confirmDeleteClass(item.id)"
                                >
                                    Delete
                                </button>
                            </div>
        ...
        <div class="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-lg bg-white p-6 shadow-lg dark:bg-gray-800">
            ...
            <form @submit.prevent="addClass" class="space-y-5">
                ...
                <div class="space-y-4">
                    <div class="grid grid-cols-1 gap-4 md:grid-cols-1">
                        ...
                        <div>
                            ...
                            <select v-model="newClass.term" required class="input-style">
                                ...
                                <option>Fifth Year - First Semester</option>
                                <option>Fifth Year - Second Semester</option>
                            </select>
                        </div>
                ...
                <div>
                    ...
                    <div v-if="formErrors.schedule" class="mt-1 text-sm text-red-600">
                        <div v-for="error in formErrors.schedule" :key="error">{{ error }}</div>
                    </div>
                ...
                <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                    <div>
                        ...
                        <input type="text" v-model="newClass.room" required placeholder="e.g., Room 205" class="input-style" />
                    </div>

                    <div>
                        <label class="mb-1 block text-sm font-medium text-gray-700 dark:text-gray-300">Max Students</label>
                        <input type="number" v-model="newClass.max_students" required placeholder="e.g., 30" class="input-style" />
                    </div>
                </div>
    ...
    <div v-if="showDeleteClassModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
        <div class="mx-4 w-full max-w-md rounded-lg bg-white p-6 shadow-lg dark:bg-gray-800">
            <h3 class="mb-4 text-lg font-medium">Confirm Delete</h3>
            <p class="mb-6 text-gray-600 dark:text-gray-300">Are you sure you want to delete this class? This action cannot be undone.</p>
            <div class="flex justify-end space-x-3">
                <button
                    @click="cancelDeleteClass"
                    class="rounded-md border border-gray-300 px-4 py-2 hover:bg-gray-100 dark:border-gray-600 dark:hover:bg-gray-700"
                >
                    Cancel
                </button>
                <button @click="proceedWithDeleteClass" class="rounded-md bg-red-600 px-4 py-2 text-white hover:bg-red-700">Delete</button>
            </div>
        </div>
    </div>
    ...
    <div v-if="showSuccessModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
        <div class="mx-4 w-full max-w-md rounded-lg bg-white p-6 shadow-lg dark:bg-gray-800">
            <h3 class="mb-4 text-lg font-medium text-green-600">Success</h3>
            <p class="mb-6 text-gray-600 dark:text-gray-300">{{ successMessage }}</p>
            <div class="flex justify-end">
                <button @click="showSuccessModal = false" class="hover:bg-primary-700 rounded-md bg-primary-600 px-4 py-2 text-white">Close</button>
            </div>
        </div>
    </div>
