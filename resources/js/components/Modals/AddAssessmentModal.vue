<template>
    <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-4">
        <div class="max-h-[90vh] w-full max-w-md overflow-y-auto rounded-lg bg-white shadow-xl dark:bg-gray-800" @click.stop>
            <div class="p-6">
                ...
                <div
                    v-if="message.text"
                    :class="`mb-4 rounded-md p-3 text-sm ${message.type === 'error' ? 'bg-red-100 text-red-700 dark:bg-red-900 dark:text-red-300' : 'bg-green-100 text-green-700 dark:bg-green-900 dark:text-green-300'}`"
                >
                    {{ message.text }}
                </div>
                ...
                <form @submit.prevent="submitForm">
                    <!-- Assessment Name -->
                    <div class="mb-4">
                        <label for="assessment_name" class="mb-1 block text-sm font-medium text-gray-700 dark:text-gray-300">Assessment Name</label>
                        <input
                            type="text"
                            id="assessment_name"
                            v-model="form.assessment_name"
                            class="w-full rounded-md border px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white"
                    ...
                    <!-- Assessment Term -->
                    <div class="mb-4">
                        <label for="term" class="mb-1 block text-sm font-medium text-gray-700 dark:text-gray-300">Assessment Term</label>
                        <select
                            id="term"
                            v-model="form.term"
                            class="w-full rounded-md border px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white"
                    ...
                    <!-- Weight -->
                    <div class="mb-6">
                        <label for="weight" class="mb-1 block text-sm font-medium text-gray-700 dark:text-gray-300">
                            Weight (%)
                        ...
                        <div class="mt-2 h-2.5 rounded-full bg-gray-200 dark:bg-gray-600">
                            <div
                    ...
                    <div class="flex justify-end gap-2">
                        <button
                            type="button"
                            @click="closeModal"
                            class="rounded-md border px-4 py-2 text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 dark:border-gray-600 dark:text-gray-300 dark:hover:bg-gray-700"
                        >
                            Cancel
                        </button>
                        <button
...
const getTotalWeightForTerm = (term) => {
    ...
    let totalWeight = assessmentsInTerm.reduce((sum, a) => sum + parseFloat(a.weight || 0), 0);
...
async function submitForm() {
    ...
    const totalTermWeight = getTotalWeightForTerm(form.term);
      ...
      if (totalTermWeight === 100 && !isEditing.value && parseFloat(form.weight) === 0) {
        errors.value.weight = `The total weight for the ${form.term} term is already 100%. You cannot add another assessment.`;
...
// Initialize form BEFORE any watches that use it
const form = useForm({
  assessment_name: "",
  term: "",
  weight: 0,
  class_id: props.class_id, // Initialize with props.class_id
})
...
// Watch for changes in currentSection - NOW form is already defined
watch(
  () => props.currentSection,
  (newSection) => {
    if (newSection) {
      currentSection.value = newSection
      form.class_id = newSection.id || props.class_id
    }
  },
  { immediate: true },
)
...
const resetForm = () => {
  ...
  form.term = ""
  form.weight = 0
  form.class_id = props.class_id // Reset to props.class_id instead of null
  isEditing.value = false
  errors.value = { assessment_name: "", term: "", weight: "" }
  message.value = { text: "", type: "" }
}
...
const getTotalWeightForTerm = (term) => {
  ...
  // Get all assessments for this class from props
  const assessmentsInClass = props.assessments || []
  ...
  if (isEditing.value && originalForm.value.term === term && originalForm.value.id) {
    totalWeight -= parseFloat(originalForm.value.weight || 0)
  }
...
async function submitForm() {
  ...
  try {
    ...
    if (error.response && error.response.data.errors) {
      ...
      if (
        error.response.data.errors.assessment_name &&
        error.response.data.errors.assessment_name.includes("already been taken")
      ) {
        message.value = {
          text: "An assessment with this name already exists. Please choose a different name.",
          type: "error",
        }
      } else {
        message.value = { text: "An error occurred. Please try again.", type: "error" }
