...
// Function to add an assessment item
function addAssessmentItem() {
  ...
  const newItem = {
    ...
    assessment_id: selectedAssessmentForItems.value.id
  };
  
  // Send the request to create the item
  router.post(route('assessment-items.store'), newItem, {
    onSuccess: () => {
      // Reset the form
      itemForm.value = { name: '', total_score: 100 };
      itemErrors.value = {};
...
// Function to delete an assessment item
