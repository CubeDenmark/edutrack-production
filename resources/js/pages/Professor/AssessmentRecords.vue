        ...
        const students =
            section.students && Array.isArray(section.students)
                ? section.students.map((student) => {
                      ...
                      const grades = {};
                      // Create a remarks object to store remarks per assessment item
                      const remarks = {};
                              ...
                              if (assessmentScore.items && Array.isArray(assessmentScore.items)) {
                                  assessmentScore.items.forEach((item) => {
                                      if (item.item_id && item.item_score !== null) {
                                          ...
                                          // Initialize remarks from item.remarks if available
                                          remarks[item.item_id] = item.remarks || '';
                                      } else {
                      ...
                      return {
                          ...
                          grades: grades,
                          remarks: remarks, // Store remarks per assessment item
                          gradeComment: student.gradeComment || '', // Keep for backward compatibility
...
// Update student remarks for a specific assessment item
function updateStudentRemarks(student, assessmentItemId, value) {
    if (!student || !currentSection.value) return;
    ...
    // Store the remarks for this specific assessment item
    student.remarks[assessmentItemId] = value;
    ...
    if (student.assessmentScores) {
        ...
        for (const assessmentScore of student.assessmentScores) {
            if (assessmentScore.items && Array.isArray(assessmentScore.items)) {
                ...
                if (itemIndex >= 0) {
                    // Update the remarks in the assessmentScores structure
                    assessmentScore.items[itemIndex].remarks = value;
                    break;
                }
            }
        }
...
function showToast(message, bgColorClass) {
    ...
    notification.className = `fixed bottom-4 right-4 ${bgColorClass} text-white px-6 py-3 rounded-lg shadow-lg transform transition-all duration-500 flex items-center z-50`;
    ...
    try {
        ...
        for (const student of currentSection.value.students) {
            ...
            const record = {
                ...
                score,
                remarks: student.remarks?.[selectedAssessmentItem.value] || '',
            };
...
function restoreGradesFromSnapshot(snapshot) {
    ...
    currentSection.value.students.forEach((student) => {
        if (snapshot[student.id]) {
            student.grades = { ...snapshot[student.id].grades };
            student.remarks = { ...snapshot[student.id].remarks };
        }
    });
}
            ...
            <div v-if="currentSection" class="space-y-6">
                            ...
                            <div class="flex flex-col justify-between gap-4 md:flex-row md:items-center">
                                ...
                                <div>
                                    <div class="flex items-center gap-4">
                                        <span class="text-5xl">{{ currentSection.icon }}</span>
                                        <div>
                                            <h1 class="text-3xl font-semibold text-black dark:text-white md:text-4xl">
                                                {{ currentSection.name }}
                                            </h1>
                                ...
                                <!-- Class Average Info -->
                                <!-- <div class="flex items-center gap-4">
                                    <div class="flex items-center gap-3 rounded-lg bg-transparent px-4 py-2 backdrop-blur-sm">
                                        <span class="text-sm font-medium text-black dark:text-gray-300">Class Average:</span>
                                        <span class="text-xl font-bold text-black dark:text-white">{{ currentSection.averageScore }}%</span>
                                        <span
                        ...
                        <div class="flex flex-col gap-4 sm:flex-row">
                            <div class="w-full">
                                ...
                                <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">
                                    Viewing assessments for
                                    {{ selectedTerm === 'Prelim' ? 'Prelim' : selectedTerm === 'Midterm' ? 'Midterm' : 'Final Term' }}
                                </p>
                            </div>
                        </div>
                    </div>
                ...
                <div class="overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-md dark:border-gray-700 dark:bg-gray-800">
                    <div class="sticky top-0 z-10 border-b border-gray-200 bg-gray-50 p-6 dark:border-gray-700 dark:bg-gray-800/50">
                        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:space-x-4">
                            <!-- Search Input -->
                            ...
                            <div class="w-full sm:w-auto">
                                <button
                                    @click="saveRecords"
                                    :disabled="isSaving"
                                    class="flex w-full items-center justify-center gap-2 rounded-lg bg-gradient-to-r from-indigo-600 to-violet-600 px-5 py-2.5 text-sm font-medium text-white shadow-md transition-colors duration-200 hover:from-indigo-700 hover:to-violet-700 disabled:opacity-70 sm:w-auto"
                                >
                                    <svg
                                        v-if="isSaving"
                        ...
                        >
                            ...
                            <div class="flex flex-col">
                                ...
                                <span class="mt-1 text-sm font-medium text-gray-900 dark:text-white">
                                    {{ selectedAssessmentItemDetails.name }}
                                </span>
                            </div>
                            ...
                            <div class="flex items-start justify-between">
                                ...
                                <div class="flex flex-col">
                                    ...
                                    <span class="mt-1 text-sm font-medium text-gray-900 dark:text-white">
                                        {{ selectedAssessmentItemDetails.totalPoints }} pts
                                    </span>
                                </div>
                        ...
                        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                            ...
                            <tbody class="divide-y divide-gray-200 bg-white dark:divide-gray-700 dark:bg-gray-800">
                                ...
                                >
                                    ...
                                    <!-- <td class="px-6 py-4 text-sm text-gray-600 dark:text-gray-400">{{ student.school_id }}</td> -->
                                    <td class="px-6 py-4">
                                        <input
                                            type="number"
                                    ...
                                    <td class="px-6 py-4">
                                        <input
                                            type="text"
                                            v-model="student.remarks[selectedAssessmentItem]"
                                            @input="updateStudentRemarks(student, selectedAssessmentItem, student.remarks[selectedAssessmentItem])"
