...
function updateStudentGrades(student) {
  ...
  student.finalGrade = calculateTermGrade(student, currentSection.value.assessments, 'Fianl Term');
...
const assessmentTotals = computed(() => {
  if (!currentSection.value || !currentSection.value.students || !currentSection.value.assessments) return {};
  
  const totals = {};
  
  // Initialize totals for each assessment
  currentSection.value.assessments.forEach(assessment => {
    totals[assessment.id] = {
      totalScore: 0,
      studentCount: 0,
      average: 0
    };
  ...
  // Calculate totals from student grades
  currentSection.value.students.forEach(student => {
    if (student.assessment_grades) {
      Object.values(student.assessment_grades).forEach((grade: any) => {
        if (totals[grade.assessment_id]) {
          totals[grade.assessment_id].totalScore += grade.total_score || 0;
          totals[grade.assessment_id].studentCount++;
        }
      });
    }
  });
  ...
  Object.keys(totals).forEach(assessmentId => {
    const total = totals[assessmentId];
    total.average = total.studentCount > 0 ? Math.round(total.totalScore / total.studentCount) : 0;
  });
...
// Computed property for grade distribution - based on current term
const gradeDistribution = computed(() => {
  ...
  (currentSection.value.students || []).forEach(student => {
    const letterGrade = getCurrentTermLetterGrade(student).toLowerCase();
    if (distribution[letterGrade] !== undefined) {
          ...
          <div class="flex flex-col sm:flex-row gap-4 w-full">
            <div class="w-full">
              ...
              <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">
                Viewing grades for {{selectedTerm === 'Prelim'? 'Prelim': selectedTerm === 'Midterm' ? 'Midterm' : 'Final Term' }} assessments
              </p>
            </div>
          </div>
        </div>
              ...
              <tr v-for="(student, index) in filteredStudents" :key="index" class="hover:bg-gray-100 dark:hover:bg-gray-700 transition">
                ...
                <td class="px-6 py-4 whitespace-nowrap">
                  <span class="text-sm font-medium text-gray-900 dark:text-white">
                    {{ isFinite(getCurrentTermGrade(student)) ? getCurrentTermGrade(student).toFixed(2) : '0.00' }}%
                  </span>
                </td>
                <td class="px-6 py-4 whitespace-nowrap">
                  <span :class="`inline-flex items-center px-3 py-1 rounded-lg text-xs font-medium ${getGradeClass(getCurrentTermLetterGrade(student))}`">
