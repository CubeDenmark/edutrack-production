  ...
  if (sections && sections.length > 0) {
    return sections.map(section => {
      ...
      return {
        ...
        studentCount: section.studentCount || fallbackSection.studentCount || 0,
        ...
        assignments: section.assignments || fallbackSection.assignments || [],
        students: section.students || fallbackSection.students || [],
      };
    });
  }
...
function sortStudents(a, b) {
  ...
  if (sortBy.value === 'name') {
    ...
    return (a.finalGrade - b.finalGrade) * direction;
  ...
  return currentSection.value.students
    .filter(student => {
      ...
      if (statusFilter.value !== 'all') {
        if (statusFilter.value !== student.status) return false;
      }

      // Filter by grade
      if (gradeFilter.value !== 'all') {
        if (gradeFilter.value !== student.letterGrade) return false;
      }
...
const filteredAssignments = computed(() => {
  ...
  return currentSection.value.assignments.filter(assignment =>
    assignment.term === selectedTerm.value
  );
});
...
const classAverage = computed(() => {
  ...
  const activeStudents = currentSection.value.students.filter(student => student.status === 'active');
  if (activeStudents.length === 0) return 0;

  const sum = activeStudents.reduce((total, student) => total + student.finalGrade, 0);
...
const gradeDistribution = computed(() => {
  ...
  currentSection.value.students.forEach(student => {
    const grade = student.letterGrade ? student.letterGrade.toLowerCase() : ""; // Ensure it's a string
...
//   currentSection.value.students.forEach(student => {
//     const grade = student.letterGrade.toLowerCase();
//     if (distribution[grade] !== undefined) {
//       distribution[grade]++;
//     }
//   });
            ...
            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
              <tr v-for="(student, index) in filteredStudents" :key="index" class="hover:bg-gray-100 dark:hover:bg-gray-700 transition">
                ...
                <td class="px-6 py-4 whitespace-nowrap">
                  <span class="text-sm font-medium text-gray-900 dark:text-white">{{ student.finalGrade }}%</span>
                </td>
                <td class="px-6 py-4 whitespace-nowrap">
                  <span :class="`inline-flex items-center px-3 py-1 rounded-lg text-xs font-medium ${getGradeClass(student.letterGrade)}`">
                    {{ student.letterGrade }}
                  </span>
                </td>
                <td class="px-6 py-4 whitespace-nowrap">
                  <span :class="{
                    'bg-emerald-100 text-emerald-800 dark:bg-emerald-900 dark:text-emerald-300': student.status === 'active',
                    'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-300': student.status === 'inactive'
...
.transition-colors {
  ...
  transition-timing-function:  background-color, border-color, text-decoration-color, fill, stroke;
  transition-timing-function: cubic-bezier(0.4, 0, 0.2, 1);
  transition-duration: 150ms;
}
...
.from-indigo-800 {
  --tw-gradient-from: rgb(55, 48, 163);
  --tw-gradient-stops: var(--tw-gradient-from), var(--tw-gradient-to, rgb(55, 48, 163, 0));
}

.to-violet-800 {
  --tw-gradient-to: rgb(91, 33, 182);
}

.from-indigo-50 {
  --tw-gradient-from: rgb(238, 242, 255);
