...
// Fallback data for sections - using the same data structure as in Grades.vue
...
const classSections = computed(() => {
  ...
  if (sections && sections.length > 0) {
    return sections.map(section => {
      ...
      // Calculate performance data from student grades
      const performance = {};
      if (students.length > 0) {
        assignments.forEach(assignment => {
          ...
          students.forEach(student => {
            if (student.grades && student.grades[assignment.id] !== undefined) {
              sum += student.grades[assignment.id];
              count++;
            }
          });
          performance[assignment.id] = count > 0 ? Math.round(sum / count) : 0;
        });
      }
...
async function addAssessmentItem() {
  ...
  try {
    ...
    // In a real app, this would be an API call to the addAssessmentItem endpoint
    // const response = await fetch(`/api/assessments/${selectedAssessmentForItems.value.id}/items`, {
    //   method: 'POST',
    //   headers: {
...
async function deleteAssessmentItem(itemId, assessmentId = null) {
  ...
  try {
    ...
    // const response = await fetch(`/api/assessment-items/${itemId}`, {
    //   method: 'DELETE'
    // });

    // if (!response.ok) {
    //   throw new Error('Failed to delete assessment item');
    // }
...
const selectedAssessmentDetails = computed(() => {
  if (!currentSection.value || !selectedAssessment.value) return null;

  return currentSection.value.assignments.find(a => a.id === selectedAssessment.value) || null;
});
...
const openModal = (assessment = null) => {
  ...
  if (assessment) {
    ...
    // Pre-select current section if available
    if (currentSection.value) {
      form.value.class_id = currentSection.value.id;
    }
  }
...
const resetForm = () => {
  form.value = {
    class_id: currentSection.value ? currentSection.value.id : '',
    name: '',
    type: '',
    weight: 0
  };
...
const getTotalWeight = (classId) => {
  if (!classId) return 0;

  const existingWeight = classWeights.value[classId] || 0;
  const formWeight = form.value.weight || 0;

  // If editing, we only add the new weight if it's different from the original
  if (isEditing.value && originalForm.value.class_id === classId) {
...
const submitForm = async () => {
  ...
  if (!form.value.class_id) {
    errors.value.class_id = 'Please select a class';
  }
  ...
  if (getTotalWeight(form.value.class_id) > 100) {
  ...
  try {
    ...
    if (isEditing.value) {
      ...
      const section = classSections.value.find(s => s.id === form.value.class_id);
      if (section) {
      ...
      // Find the section and add the new assignment
      const section = classSections.value.find(s => s.id === form.value.class_id);
      if (section) {
        ...
        <div class="p-6">
          ...
          <form @submit.prevent="submitForm">
            ...
            <div class="mb-4">
              ...
              <p v-if="errors.class_id" class="mt-1 text-sm text-red-600 dark:text-red-400">{{ errors.class_id }}</p>
            </div>
            ...
            <div class="mb-6">
              <label for="weight" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                Weight (%)
                <span v-if="classWeights[form.class_id]" class="text-sm text-gray-500 dark:text-gray-400">
                  Current total: {{ (classWeights[form.class_id] || 0) + (isEditing ? 0 : form.weight) }}%
                </span>
              </label>
              <input
                type="number"
              ...
              <div v-if="form.class_id" class="mt-2 bg-gray-200 dark:bg-gray-600 rounded-full h-2.5">
                <div
                  class="h-2.5 rounded-full"
                  :class="getTotalWeight(form.class_id) > 100 ? 'bg-red-600' : 'bg-indigo-600'"
                  :style="`width: ${Math.min(getTotalWeight(form.class_id), 100)}%`"
                ></div>
              </div>
              <p v-if="form.class_id" class="text-xs text-gray-500 dark:text-gray-400 mt-1">
                Remaining: {{ Math.max(0, 100 - getTotalWeight(form.class_id)) }}%
              </p>
            </div>
