...
import { Edit3, ClipboardList, AlertTriangle, Calendar, CheckCircle, ChevronDown, Clock, FileText, Users } from 'lucide-vue-next';
...
// Get schedule days for the current section
const currentScheduleList = computed(() => {
    if (!currentSection.value || !currentSection.value.schedule) return [];

    try {
        const parsed = JSON.parse(currentSection.value.schedule);
...
// Navigate to a section with a specific ID - FIXED FUNCTION
function navigateToSection(sectionId) {
    ...
    if (sectionId === 'attendance') {
        window.location.href = '/prof/attendance?sectionId=' + currentSection.value.id;
    } else if (sectionId === 'top-performers' || sectionId === 'grades') {
        window.location.href = '/prof/grades?itemId=' + currentSection.value.id;
    } else if (sectionId === 'behavior-reports') {
        window.location.href = '/prof/behavior_reports?itemId=' + currentSection.value.id;
    } else if (sectionId === 'schedule') {
        window.location.href = '/prof/schedule?itemId=' + currentSection.value.id;
    } else if (sectionId === 'assessment-records') {
        window.location.href = '/prof/assess_records?itemId=' + currentSection.value.id;
    } else if (sectionId === 'assessment') {
        window.location.href = '/prof/assessment?itemId=' + currentSection.value.id;
    } else {
        // In-page navigation fallback
        url.searchParams.set('sectionId', currentSection.value.id);
        window.history.replaceState({}, '', url.toString());
...
// Initialize refs for event listeners
const itemChangeHandler = ref(null);
const sectionChangeHandler = ref(null);
...
// Lifecycle hooks
onMounted(() => {
    // Assign the event listener functions to the refs
    itemChangeHandler.value = handleItemChange;
    sectionChangeHandler.value = handleSectionChange;

    // Add event listener for item changes from sidebar
    window.addEventListener('item-changed', itemChangeHandler.value);

    // For backward compatibility, also listen for section-changed events
    window.addEventListener('section-changed', sectionChangeHandler.value);
    ...
    return () => {
        window.removeEventListener('item-changed', itemChangeHandler.value);
        window.removeEventListener('section-changed', sectionChangeHandler.value);
    };
});
</script>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="p-6 pt-20">
            ...
            <div v-if="sections.length === 0" class="flex h-[calc(100vh-10rem)] flex-col items-center justify-center text-center">
                ...
                <p class="mb-6 max-w-md text-gray-500 dark:text-gray-400">
                    You don't have any classes yet. Please click the '+' button to add your classes.
                </p>
            </div>
            ...
            <div v-if="currentSection" class="space-y-6">
                ...
                <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
                    ...
                    <!-- Schedule Card (Replaced Top Performers) -->
                    <div
                        id="schedule"
                        class="rounded-lg border border-gray-100 bg-white p-6 shadow-sm transition-all hover:shadow-md dark:border-gray-700 dark:bg-gray-800"
                    >
                        ...
                        <div class="mt-4 text-sm">
                            <div
                                v-for="(item, index) in currentScheduleList"
                                :key="index"
                                class="mb-1 rounded-lg bg-purple-100 px-3 py-1 text-xs font-medium text-purple-600 dark:bg-purple-900/30 dark:text-purple-400"
                            >
                                {{ item.day }}: {{ item.time }}
                            </div>
                        </div>
                ...
                <div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
                    ...
                    >
                        ...
                        <div class="dark:bg-gray-750 border-t border-gray-100 bg-gray-50 px-6 py-3 dark:border-gray-700">
                            <a
                                href="/prof/grades"
                                @click.prevent="navigateToSection('grades')"
                        ...
                        <div class="dark:bg-gray-750 border-t border-gray-100 bg-gray-50 px-6 py-3 dark:border-gray-700">
                            <a
                                href="/prof/behavior_reports"
                                 @click.prevent="navigateToSection('behavior-reports')"
                                class="text-sm font-medium text-indigo-600 hover:text-indigo-700 dark:text-indigo-400 dark:hover:text-indigo-300"
                            >
                                View all reports →
                            </a>
                ...
                <!-- Attendance Stats -->
                <div id="attendance" class="rounded-lg border border-gray-100 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
                ...
                <!-- Quick Actions -->
                <div id="quick-actions" class="rounded-lg border border-gray-100 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
                    <h2 class="mb-4 text-lg font-semibold">Quick Actions</h2>
                    <div class="grid grid-cols-2 gap-4 md:grid-cols-5">
                        <!-- Take Attendance -->
                        <a
                            href="/prof/attendance"
                            @click.prevent="navigateToSection('attendance')"
                        ...
                        <a
                            href="/prof/grades"
                            @click.prevent="navigateToSection('top-performers')"
                            class="flex flex-col items-center rounded-lg border border-gray-200 bg-white p-4 transition-all hover:bg-gray-50 hover:shadow-md dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-700"
                        >
                            <div
                        ...
                        <!-- Assessment Records -->
                        <a
                            href="/prof/assessment_records"
                            @click.prevent="navigateToSection('assessment-records')"
                            class="flex flex-col items-center rounded-lg border border-gray-200 bg-white p-4 transition-all hover:bg-gray-50 hover:shadow-md dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-700"
                        >
                            <div
                            ...
                            <span class="text-sm font-medium">Assessment Records</span>
                        </a>

                        <!-- Assessment -->
                        <a
                            href="/prof/assessment"
                            @click.prevent="navigateToSection('assessment')"
                            class="flex flex-col items-center rounded-lg border border-gray-200 bg-white p-4 transition-all hover:bg-gray-50 hover:shadow-md dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-700"
                        >
                            <div
                                class="mb-3 flex h-12 w-12 items-center justify-center rounded-full bg-indigo-100 text-indigo-600 dark:bg-indigo-900/30 dark:text-indigo-400"
                            >
                                <Edit3 class="h-6 w-6" />
                            </div>
