...
const classes = computed(() => {
    const classData = usePage().props.classes || [];

    if (!classData || !Array.isArray(classData)) {
        console.error('Classes data is not available or not an array:', classData);
        return [];
    }
    ...
    return classData.map(cls => {
        if (cls.schedule && typeof cls.schedule === 'string') {
            try {
                cls.schedule = JSON.parse(cls.schedule);
            } catch (e) {
...
const getClassSchedule = (cls) => {
  ...
  if (cls.schedule) {
    if (typeof cls.schedule === 'string') {
      try {
        return JSON.parse(cls.schedule);
      } catch (e) {
        console.error('Failed to parse schedule data:', e);
        return [];
      }
...
onMounted(() => {
  ...
  let initialClassSelected = false;
  ...
  if (!initialClassSelected && classes.value.length > 0) {
...
<template>
  ...
  <AppLayout :breadcrumbs="breadcrumbs">
    ...
    <div v-if="!currentClass" class="flex flex-col items-center justify-center h-[calc(100vh-10rem)] text-center">
      ...
      <div  v-if="getClassSchedule(currentClass).length > 0" class="bg-white dark:bg-gray-800 rounded-lg shadow p-6 mb-6">
        <div class="flex items-center">
          <span class="text-3xl mr-4">{{ currentClass.icon }}</span>
          <div>
            <h3 class="text-lg font-medium">{{ currentClass.name }}</h3>
            <p class="text-gray-500 dark:text-gray-400">{{ currentClass.teacher }} | {{ currentClass.room }}</p>
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">
              Class Days:
        <span v-for="(item, index) in getClassSchedule(currentClass)" :key="index">
          {{ item.day }} ({{ item.time }})<span v-if="index !== getClassSchedule(currentClass).length - 1">, </span>
        </span>
            </p>
          </div>
        </div>
            ...
            <div class="relative">
              ...
              >
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                  <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1. 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clip-rule="evenodd" />
                </svg>
              </button>
            </div>
            <select
